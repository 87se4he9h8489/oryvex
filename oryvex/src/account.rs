...
pub fn generate_masque_keypair() -> Result<MasqueKeyPair> {
    ...
    let ec = EcKey::generate(&group).map_err(|e| OryvexError::Tls(e.to_string()))?;
    let pkey = PKey::from_ec_key(ec).map_err(|e| OryvexError::Tls(e.to_string()))?;
    ...
    let mut builder = X509Builder::new().map_err(|e| OryvexError::Tls(e.to_string()))?;
    builder
        .set_version(2)
        .map_err(|e| OryvexError::Tls(e.to_string()))?;
    ...
    let not_before = Asn1Time::days_from_now(0).map_err(|e| OryvexError::Tls(e.to_string()))?;
    let not_after = Asn1Time::days_from_now(1).map_err(|e| OryvexError::Tls(e.to_string()))?;
    builder
...
fn http_client() -> Result<reqwest::Client> {
    reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| OryvexError::Api(e.to_string()))
}
...
pub async fn register(model: &str, locale: &str, jwt: Option<&str>) -> Result<(AccountData, [u8; 32])> {
    ...
    let resp = req.send().await.map_err(|e| OryvexError::Api(e.to_string()))?;
...
async fn parse_account(resp: reqwest::Response) -> Result<AccountData> {
    ...
    let text = resp.text().await.map_err(|e| OryvexError::Api(e.to_string()))?;

    if !status.is_success() {
        return Err(OryvexError::Api(format!("status {status}: {text}")));
    }

    serde_json::from_str::<AccountData>(&text)
        .map_err(|e| OryvexError::Api(format!("decode: {e}; body={text}")))
}

fn extract_wg_peer(reg: &AccountData) -> Result<[u8; 32]> {
    if reg.config.peers.is_empty() {
        return Err(OryvexError::Api("no peers in registration response".into()));
    }
    ...
    let decoded = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, peer_b64)
        .map_err(|e| OryvexError::Api(format!("decode peer pubkey: {e}")))?;
    if decoded.len() != 32 {
        return Err(OryvexError::Api("invalid peer pubkey length".into()));
    }
    let mut arr = [0u8; 32];
...
pub async fn provision_wg(model: &str, locale: &str, jwt: Option<&str>) -> Result<Identity> {
    ...
    if reg.token.is_empty() {
        return Err(OryvexError::Api("registration returned empty token".into()));
    }
