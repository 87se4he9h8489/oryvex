...
async fn run_cli() -> Result<()> {
    ...
    let listen: SocketAddr = std::env::var("ORYVEX_SOCKS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or_else(|| "127.0.0.1:1819".parse().unwrap());

    let base_config = std::env::var("ORYVEX_CONFIG").unwrap_or_else(|_| DEFAULT_CONFIG.to_string());

    let protocol = if std::env::var("ORYVEX_PEER").is_ok() || std::env::var("ORYVEX_WG_PEER").is_ok() {
        match std::env::var("ORYVEX_PROTOCOL") {
...
fn noize_config() -> noize::NoizeConfig {
    let profile = std::env::var("ORYVEX_NOIZE").unwrap_or_else(|_| "firewall".to_string());
...
fn masque_config_path(base: &str) -> String {
    if let Ok(p) = std::env::var("ORYVEX_MASQUE_CONFIG") {
        return p;
    }
...
async fn select_peer(identity: &account::Identity, protocol: Protocol) -> Result<SocketAddr> {
    let force_peer = match protocol {
        Protocol::Masque => std::env::var("ORYVEX_PEER").ok(),
        Protocol::WireGuard | Protocol::WarpInWarp => std::env::var("ORYVEX_WG_PEER")
            .ok()
            .or_else(|| std::env::var("ORYVEX_PEER").ok()),
    };
...
async fn resolve_ech() -> Option<Vec<u8>> {
    match std::env::var("ORYVEX_ECH") {
        ...
        Ok(b64) if !b64.is_empty() => match tls::decode_ech_config_list(&b64) {
            Ok(v) => {
                log::info!("[+] using ECHConfigList from ORYVEX_ECH");
                Some(v)
            }
            Err(e) => {
                log::warn!("[-] bad ORYVEX_ECH: {e}; continuing without ECH");
                None
            }
        },
        _ => {
...
async fn run_masque_tunnel(
    ...
    match tunnel_result {
        Ok(()) => Ok(()),
        Err(e) => Err(OryvexError::Other(format!("tunnel exited: {e}"))),
    }
}

fn wg_keepalive_secs() -> u16 {
    std::env::var("ORYVEX_WG_KEEPALIVE")
        .ok()
...
async fn run_wireguard(identity: account::Identity, listen: SocketAddr) -> Result<()> {
    ...
    let forced = std::env::var("ORYVEX_WG_PEER")
        .ok()
        .or_else(|| std::env::var("ORYVEX_PEER").ok());
    ...
    let (peer, profile) = selected.ok_or(OryvexError::NoCleanEndpoint)?;
    log::info!("[+] using cloudflare edge {peer}");
...
async fn run_wireguard_tunnel(
    ...
    match test_result {
        ...
        Err(e) => {
            ...
            return Err(OryvexError::Other(format!("WireGuard handshake failed: {e}")));
        }
    }
    ...
    match tunnel_result {
        Ok(()) => Ok(()),
        Err(e) => Err(OryvexError::Other(format!("wireguard tunnel exited: {e}"))),
    }
}
...
async fn select_scan_mode() -> prober::ScanMode {
    if let Ok(v) = std::env::var("ORYVEX_SCAN") {
...
async fn select_protocol() -> Protocol {
    if let Ok(v) = std::env::var("ORYVEX_PROTOCOL") {
...
async fn select_ip_version() -> prober::IpScan {
    if let Ok(v) = std::env::var("ORYVEX_IP") {
