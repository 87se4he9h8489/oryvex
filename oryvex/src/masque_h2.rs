...
pub fn enabled() -> bool {
    match std::env::var("ORYVEX_MASQUE_HTTP2") {
        Ok(v) => {
...
pub fn h2_peer(quic_peer: SocketAddr) -> SocketAddr {
    if let Ok(v) = std::env::var("ORYVEX_MASQUE_H2_PEER") {
...
fn build_tls(cfg: &H2TunnelConfig) -> Result<boring::ssl::ConnectConfiguration> {
    let mut builder =
        SslConnector::builder(SslMethod::tls()).map_err(|e| OryvexError::Tls(e.to_string()))?;

    builder
        .set_min_proto_version(Some(SslVersion::TLS1_2))
        .map_err(|e| OryvexError::Tls(e.to_string()))?;
    builder
    ...
    let groups = std::env::var("ORYVEX_TLS_GROUPS").ok();
    let groups = groups
        .as_deref()
        .map(str::trim)
        .filter(|s| !s.is_empty())
    ...
    let cert = X509::from_pem(&cfg.cert_pem).map_err(|e| OryvexError::Tls(e.to_string()))?;
    let key =
        PKey::private_key_from_pem(&cfg.key_pem).map_err(|e| OryvexError::Tls(e.to_string()))?;
    builder
...
fn build_connect_request(cfg: &H2TunnelConfig) -> Result<http::Request<()>> {
    ...
    http::Request::builder()
        ...
        .header("user-agent", "")
        .body(())
        .map_err(|e| OryvexError::Masque(format!("build request: {e}")))
}
...
pub async fn verify_h2(cfg: &H2TunnelConfig, timeout: Duration) -> Result<Duration> {
    ...
    let attempt = async {
        ...
        let tcp = TcpStream::connect(cfg.peer).await.map_err(OryvexError::Io)?;
        let _ = tcp.set_nodelay(true);
        let tls = tokio_boring::connect(tls_config, &cfg.sni, tcp)
            .await
            .map_err(|e| OryvexError::Tls(format!("h2 tls handshake: {e}")))?;
        let (h2, connection) = h2::client::handshake(tls)
            .await
            .map_err(|e| OryvexError::Masque(format!("h2 handshake: {e}")))?;
        ...
        let mut h2 = h2
            .ready()
            .await
            .map_err(|e| OryvexError::Masque(format!("h2 ready: {e}")))?;
        let req = build_connect_request(cfg)?;
        let (resp_fut, _send_stream) = h2
            .send_request(req, false)
            .map_err(|e| OryvexError::Masque(format!("send_request: {e}")))?;
        let response = resp_fut
            .await
            .map_err(|e| OryvexError::Masque(format!("await response: {e}")))?;
        driver.abort();
        let status = response.status();
        if !status.is_success() {
            return Err(OryvexError::Masque(format!(
    ...
    match tokio::time::timeout(timeout, attempt).await {
        ...
        Ok(Err(e)) => Err(e),
        Err(_) => Err(OryvexError::Other("h2 verify timeout".into())),
    }
}
            ...
            data = futures::future::poll_fn(|cx| recv_body.poll_data(cx)) => {
                match data {
                    ...
                    Some(Err(e)) => {
                        ...
                        return Err(OryvexError::Masque(format!("h2 body: {e}")));
                    }
                    None => {
...
async fn send_capsule(send: &mut h2::SendStream<Bytes>, data: Bytes) -> Result<()> {
    ...
    loop {
        match futures::future::poll_fn(|cx| send.poll_capacity(cx)).await {
            ...
            Some(Err(e)) => return Err(OryvexError::Masque(format!("h2 capacity: {e}"))),
            None => return Err(OryvexError::Masque("h2 stream closed".into())),
        }
    }

    send.send_data(data, false)
        .map_err(|e| OryvexError::Masque(format!("h2 send_data: {e}")))?;
    Ok(())
}
