...
async fn handle_client(mut sock: TcpStream, stack: StackHandle) -> Result<()> {
    ...
    if head[0] != VER {
        return Err(OryvexError::Other("bad socks version".into()));
    }
    ...
    match cmd {
        ...
        _ => {
            ...
            Err(OryvexError::Other("unsupported socks command".into()))
        }
    }
}
...
async fn handshake(sock: &mut TcpStream) -> Result<()> {
    ...
    if prefix[0] != VER {
        return Err(OryvexError::Other("bad greeting version".into()));
    }
...
async fn read_target(sock: &mut TcpStream, atyp: u8) -> Result<(Target, u16)> {
    let target = match atyp {
        ...
        _ => return Err(OryvexError::Other("bad atyp".into())),
    };
...
async fn dns_resolve(stack: &StackHandle, name: &str) -> Result<IpAddr> {
    ...
    let resp = tokio::time::timeout(Duration::from_secs(5), from_stack.recv())
        .await
        .map_err(|_| OryvexError::Other("dns timeout".into()))?
        .ok_or_else(|| OryvexError::Other("dns channel closed".into()))?;

    parse_dns_a(&resp.1).ok_or_else(|| OryvexError::Other(format!("no A record for {name}")))
}
