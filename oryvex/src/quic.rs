...
use crate::{consts, error::OryvexError, error::Result};
...
async fn bind_udp_fast(bind_addr: SocketAddr) -> Result<UdpSocket> {
    ...
    let sock = Socket::new(domain, Type::DGRAM, None).map_err(OryvexError::Io)?;
    sock.set_nonblocking(true).map_err(OryvexError::Io)?;
    
    let buf_size = 7 * 1024 * 1024; // 7MB
    let _ = sock.set_recv_buffer_size(buf_size);
    let _ = sock.set_send_buffer_size(buf_size);
    
    sock.bind(&bind_addr.into()).map_err(OryvexError::Io)?;
    UdpSocket::from_std(sock.into()).map_err(OryvexError::Io)
}
...
fn poll_h3(
    ...
    loop {
        match h3c.poll(conn) {
            ...
            Err(e) => return Err(OryvexError::H3(e)),
        }
    }
...
async fn flush(
    ...
    loop {
        match conn.send(&mut out) {
            ...
            Err(quiche::Error::Done) => break,
            Err(e) => return Err(OryvexError::Quic(e)),
        }
    }
...
async fn do_migrate(
    ...
    if conn.available_dcids() == 0 {
        return Err(OryvexError::Other("no spare dcids for migration".into()));
    }
...
pub async fn verify_masque(p: &VerifyParams) -> Result<Duration> {
    ...
    loop {
        ...
        tokio::select! {
            r = sock.recv(&mut buf) => {
                match r {
                    ...
                    Err(e) => return Err(OryvexError::Io(e)),
                }
            }
                    ...
                    Ok((stream_id, h3::Event::Headers { list, .. })) if stream_id == sid => {
                        for h in &list {
                            if h.name() == b":status" {
                                ...
                                return Err(OryvexError::Other(format!(
                                    "status {}",
        ...
        if conn.is_closed() {
            return Err(OryvexError::Other("closed before 200".into()));
        }
    }
}
