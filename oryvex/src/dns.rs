...
pub async fn fetch_ech_config() -> Result<Vec<u8>> {
    ...
    Err(OryvexError::Ech("no ECHConfigList resolved".into()))
}
...
async fn query_ech(server: SocketAddr, host: &str) -> Result<Vec<u8>> {
    ...
    let n = timeout(Duration::from_secs(3), sock.recv(&mut buf))
        .await
        .map_err(|_| OryvexError::Ech("dns timeout".into()))??;

    parse_https_ech(&buf[..n]).ok_or_else(|| OryvexError::Ech("no ech svcparam".into()))
}
