...
use crate::oryvexnoize::{self, OryvexNoizeConfig};
use crate::error::{OryvexError, Result};
use rand::Rng;
...
pub struct WgConfig {
    ...
    pub oryvexnoize: Arc<OryvexNoizeConfig>,
}
...
impl WgTunnel {
    pub async fn new(cfg: WgConfig, inbound_tx: mpsc::Sender<Vec<u8>>) -> Result<Self> {
        ...
        let tunn = Tunn::new(local_secret, peer_public, preshared, cfg.persistent_keepalive, 0, None)
            .map_err(|e| OryvexError::Other(format!("wireguard tunnel init: {e}")))?;
        ...
        Ok(Self {
            ...
            oryvexnoize: cfg.oryvexnoize.clone(),
    ...
    pub async fn run(self, mut outbound_rx: mpsc::Receiver<Vec<u8>>) -> Result<()> {
        ...
        let oryvexnoize = self.oryvexnoize.clone();
        let oryvexnoize_t = self.oryvexnoize.clone();
                ...
                match tunn.encapsulate(&ip_packet, &mut out_buf) {
                    ...
                    TunnResult::WriteToNetwork(pkt) => {
...

                        {
                            ...
                            if !*sent && oryvexnoize.is_enabled() {
                                *sent = true;
                                drop(sent);
                                oryvexnoize::apply_obfuscation(&sock_w, peer, &oryvexnoize).await;
                            }
                        }

                        let _ = sock_w.send(&pkt_vec).await;

                        if oryvexnoize.jc_after_hs > 0 {
                            let sock_clone = sock_w.clone();
                            let cfg_clone = oryvexnoize.clone();
                            tokio::spawn(async move {
                                oryvexnoize::send_post_handshake_junk(&sock_clone, peer, &cfg_clone).await;
                            });
                        }
                    }
        ...
        let timer_task = tokio::spawn(async move {
            ...
            loop {
                ...
                if let TunnResult::WriteToNetwork(pkt) = tunn.update_timers(&mut tmp) {
                    ...
                    if oryvexnoize_t.is_enabled() {
                        let sock_j = sock_t.clone();
                        let cfg_j = oryvexnoize_t.clone();
                        tokio::spawn(async move {
                            oryvexnoize::send_keepalive_junk(&sock_j, &cfg_j).await;
...
async fn send_dataplane_probe(
    ...
    match tunn.encapsulate(probe, out_buf) {
        ...
        TunnResult::Err(e) => {
            return Err(OryvexError::Other(format!("dataplane encap: {e:?}")));
        }
        _ => {}
    }
    Ok(())
...
async fn verify_dataplane(
    ...
    loop {
        ...
        if now >= deadline {
            ...
            return Err(OryvexError::Other("dataplane timeout".into()));
        }
        if now >= resend_at {
...
pub async fn verify_endpoint(
    ...
    client_id: [u8; 3],
    local_ipv4: Ipv4Addr,
    oryvexnoize: &OryvexNoizeConfig,
    timeout: Duration,
) -> Result<Duration> {
    let data_check = std::env::var("ORYVEX_WG_NO_DATA_CHECK").is_err();
    log::debug!("[wg] verify {} obf={} data_check={}", peer, oryvexnoize.is_enabled(), data_check);
    ...
    if oryvexnoize.is_enabled() {
        oryvexnoize::apply_obfuscation(&sock, peer, oryvexnoize).await;
    }
    ...
    let mut tunn = Tunn::new(local_secret, peer_pk, None, Some(25), 0, None)
        .map_err(|e| OryvexError::Other(format!("tunn init: {e}")))?;
    ...
    match tunn.encapsulate(&[], &mut out_buf) {
        ...
        other => {
            ...
            return Err(OryvexError::Other("handshake init failed".into()));
        }
    }
    ...
    loop {
        if Instant::now() >= deadline {
            ...
            return Err(OryvexError::Other("verify timeout".into()));
        }
