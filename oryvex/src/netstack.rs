...
impl TcpConn {
    pub async fn send(&self, data: Vec<u8>) -> Result<()> {
        self.data_in
            ...
            .await
            .map_err(|_| OryvexError::Other("netstack closed".into()))
    }
...
impl StackHandle {
    pub async fn open_tcp(&self, dst: SocketAddr) -> Result<TcpConn> {
        ...
        resp_rx
            .await
            .map_err(|_| OryvexError::Other("netstack dropped".into()))?
            .map_err(OryvexError::Other)
    }
...
fn parse_v4(s: &str) -> Result<Option<(Ipv4Addr, u8)>> {
    ...
    let ip: Ipv4Addr = strip_cidr(s)
        .parse()
        .map_err(|_| OryvexError::Other(format!("bad ipv4 {s}")))?;
...
fn parse_v6(s: &str) -> Result<Option<(Ipv6Addr, u8)>> {
    ...
    let ip: Ipv6Addr = strip_cidr(s)
        .parse()
        .map_err(|_| OryvexError::Other(format!("bad ipv6 {s}")))?;
