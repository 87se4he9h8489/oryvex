...
pub fn from_profile(name: &str) -> OryvexNoizeConfig {
    match name {
        "off" | "none" => OryvexNoizeConfig::off(),
        "light" => OryvexNoizeConfig::light(),
        "aggressive" | "heavy" => OryvexNoizeConfig::aggressive(),
        _ => OryvexNoizeConfig::balanced(),
    }
}
...
fn generate_junk(cfg: &OryvexNoizeConfig) -> Vec<u8> {
...
pub async fn apply_obfuscation(sock: &UdpSocket, _peer: SocketAddr, cfg: &OryvexNoizeConfig) {
    if !cfg.is_enabled() {
        return;
    }
...
pub async fn send_post_handshake_junk(sock: &UdpSocket, _peer: SocketAddr, cfg: &OryvexNoizeConfig) {
...
pub async fn send_keepalive_junk(sock: &UdpSocket, cfg: &OryvexNoizeConfig) {
    if !cfg.is_enabled() {
        return;
    }
