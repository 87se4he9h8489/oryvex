...
pub fn build_config(params: &TlsParams) -> Result<quiche::Config> {
    ...
    let cert = X509::from_pem(params.cert_pem).map_err(|e| OryvexError::Tls(e.to_string()))?;
...
pub fn inject_ech(conn: &mut quiche::Connection, ech_config_list: &[u8]) -> Result<()> {
    if ech_config_list.is_empty() {
        return Err(OryvexError::Ech("empty ech config list".into()));
    }
    ...
    if rc != 1 {
        return Err(OryvexError::Ech(format!(
...
pub fn decode_ech_config_list(b64: &str) -> Result<Vec<u8>> {
    ...
    base64::engine::general_purpose::STANDARD
        .decode(b64.trim())
        .map_err(|e| OryvexError::Ech(e.to_string()))
}