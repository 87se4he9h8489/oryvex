...
readonly REPO="CluvexStudio/Oryvex"
readonly BIN_NAME="oryvex"
...
do_install() {
  ...
  local arch
  arch="$(detect_arch)"
  local archive="oryvex-android-${arch}.tar.gz"
  ...
  if [[ -f "${VERSION_FILE}" ]] && [[ "$(cat "${VERSION_FILE}")" == "${tag_name}" ]]; then
    success "Oryvex ${tag_name} is already installed. Nothing to do."
    return 0
  fi

  TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/oryvex-install.XXXXXX")"
  ...
  success "Oryvex ${tag_name} installed. Run it with: ${BIN_NAME}"
  info "Once running, SOCKS5 proxy will listen on 127.0.0.1:1819"
}

do_update() {
  if [[ ! -f "${VERSION_FILE}" ]]; then
    warn "Oryvex is not installed yet. Installing latest release instead."
    do_install "latest"
    return
  fi
...
do_uninstall() {
  if [[ -f "${INSTALL_PATH}" ]]; then
    ...
    success "Oryvex has been uninstalled."
  else
    warn "Oryvex is not installed."
  fi
}
...
show_menu() {
  clear
  echo -e "${GREEN}=== Oryvex Termux Installer ===${RESET}"
  do_status
  echo ""
