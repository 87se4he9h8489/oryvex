...
# Complete Guide to Oryvex

This document tries to gather in one place everything you need to work with Oryvex. It is written plainly, step by step, so that even if this is your first time you will not get lost.

## What Oryvex actually does

Oryvex is a tunnel. Its job is to open an encrypted path out of a restricted network and bring up a local proxy next to you called SOCKS5. After that, any application that knows how to go through a proxy — a browser, a terminal, or your whole system — sends its traffic through this tunnel.

The proxy's default address is `127.0.0.1:1819`.

## Three transports, three different logics

When you run Oryvex, the first thing it asks is which protocol to use. You have three choices:
...
This is the most important part that keeps Oryvex alive on a strict network.
...
MASQUE in Oryvex has two paths to carry the traffic:
...
```
ORYVEX_MASQUE_HTTP2=1 ./target/release/oryvex
```
...
Oryvex has a watchdog that watches the flow of data. If it sees that you are sending but for a long time (default 20 seconds) nothing comes back, it concludes that the path is dead; it tears it down and automatically reconnects to a freshly scanned endpoint. When you have no traffic (idle), it does not reconnect pointlessly.

## Full table of environment variables

Every prompt has a variable equivalent. If you set a variable beforehand, Oryvex no longer asks that question. This is excellent for automated runs and scripting.

### General selection

- `ORYVEX_PROTOCOL` — protocol: `masque`, `wg`, or `gool`.
- `ORYVEX_SOCKS` — the proxy listen address. Default `127.0.0.1:1819`.
- `ORYVEX_NOIZE` — obfuscation profile (explained above).
- `ORYVEX_SCAN` — scan mode: `turbo`, `balanced`, `thorough`, `stealth`.
- `ORYVEX_IP` — IP version for scanning: IPv4, IPv6, or both.

### Specific to MASQUE

- `ORYVEX_MASQUE_HTTP2` — if it is `1`/`true`/`h2`/`yes`/`on`, it uses h2. Otherwise h3.
- `ORYVEX_MASQUE_H2_PEER` — manual override of the destination address for h2 mode.

### Specific to WireGuard and gool

- `ORYVEX_WG_KEEPALIVE` — the keepalive packet interval in seconds. Default `5`.
- `ORYVEX_WG_STALL` — the threshold for detecting a stall and reconnecting, in seconds. Default `20`.
- `ORYVEX_NO_WATCHDOG` — if set, the watchdog and automatic reconnection are turned off.
- `ORYVEX_WG_NO_DATA_CHECK` — if set, real data passage is not verified during the scan (faster but less reliable).
- `ORYVEX_WG_NO_PROFILE_RETRY` — if set, on a failed scan it does not retry with other noise profiles.

### Forcing the endpoint and the config path

- `ORYVEX_PEER` or `ORYVEX_WG_PEER` — if you want to give a fixed address yourself and bypass the scan.
- `ORYVEX_CONFIG` — the path of the base config file. Default `oryvex.toml`.
- `ORYVEX_WG_CONFIG` and `ORYVEX_MASQUE_CONFIG` — the config path specific to each protocol.
...
```
./target/release/oryvex
```

### MASQUE on h2 for a network that has blocked UDP

```
ORYVEX_PROTOCOL=masque ORYVEX_MASQUE_HTTP2=1 ORYVEX_NOIZE=firewall ./target/release/oryvex
```

### Fast WireGuard on a strict network

```
ORYVEX_PROTOCOL=wg ORYVEX_NOIZE=aggressive ORYVEX_SCAN=thorough ./target/release/oryvex
```

### gool with a custom port

```
ORYVEX_PROTOCOL=gool ORYVEX_SOCKS=127.0.0.1:1080 ./target/release/oryvex
```
...
If you want it in one sentence: start from MASQUE with the default profile, if UDP is blocked turn on h2, and if it is still strict, make the noise profile heavier or move to WireGuard and gool. Oryvex takes care of the rest.
